    ...
    if let Some(command) = &cli.command {
        match command {
            Commands::Export { provider, force } => {
                ...
                let snapshot = match provider.export_library().await {
                    Ok(snapshot) => {
            ...
            Commands::Sync { from, to, force } => {
                ...
                let snapshot = match source.export_library().await {
                    Ok(snapshot) => {
                        storage::clear_provider_cooldown(source.kind())?;
                        snapshot
                    }
                    Err(error) => {
                        remember_provider_failure(source.kind(), &error);
                        return Err(error);
                    }
                };
                let merge_summary = merge_provider_snapshot(&mut state, snapshot);
                ...
                let summary = if *force {
                    ...
                    let mut preview = state.clone();
                    match destination.sync_library(&mut preview, false).await {
                        Ok(summary) => summary,
                        Err(error) => {
                            remember_provider_failure(destination.kind(), &error);
                            return Err(error);
                        }
                    }
                };
