...
If a discovered provider ID already belongs to another canonical row and both rows also contain conflicting IDs on another provider, the app does not auto-merge. Use the Conflicts page in the web app to compare the source row and candidate owner row; each conflict includes metadata similarity, duration delta, provider confidence when available, saved/playlist impact, and a conservative review recommendation. The queue can be filtered by candidate provider, recommendation, and whether the row affects saved tracks or playlists, and it prioritizes likely-same conflicts that have library impact. The same page can plan and run a guarded bulk merge for rows still classified as likely-same; it creates a manual source-of-truth backup first, re-checks every row before merging, and never mutates Spotify or YouTube Music accounts. For individual or bulk merges, choose whether source provider IDs or candidate provider IDs win. If the candidate is the wrong recording, mark it as not the same track; the rejected candidate is recorded so the next identity run does not immediately re-open the same conflict.
